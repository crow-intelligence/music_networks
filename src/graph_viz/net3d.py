...
class netSimple:
    def __init__(self, pts, edg, directed=False, **kw):
        ...
        self.params = {"A": 200, "pow": POWn, "n_radius": 1.0, "k": 10}
    ...
    def comp_F_NL(self):
        k = self.params["k"]
        self.F_NL = (
            -k * self.dp * self.Adj[:, :, newaxis]
        )  # weights also determine strength of springs
...
class netRads(netSimple):
    def _init(self, kw):
        ...
        self.params["links"] = {
            "T0": self.params["nodes"]["radius"] / 2.0,
            "ce": 100.0,
            "weighted": True,
        }
        if "nodes" in kw:
            self.params["nodes"].update(kw["nodes"])
        if "links" in kw:
            self.params["links"].update(kw["links"])
    ...
    def set_radii(self):
        d = self.pts.shape[-1]
        rs = (
            lambda x: ((x ** (d - 1)).sum(-1)) ** (1.0 / (d - 1)) + 1e-5
        )  # use root sum squared of weights as radii for 3D
        self.degreesRS = r0 = (
            rs(self.Adj)
            if self.params["nodes"]["weighted"]
            else array([1] * len(self.Adj))
        )

        self.rad_mat = (
            self.params["nodes"]["radius"] * (r0 + r0[:, newaxis])[:, :, newaxis]
        )
    ...
    def comp_f_long(self):
        ...
        self.f_long = (
            self.params["long_rep"]
            * self.params["A"]
            * self.dp
            / (self.dpn ** (d - 1) + 1e-3) ** (d / (d - 1))
        )

    def comp_thermal_noise(self):
        self.noise = (
            self.params["links"]["T0"]
            * exp(-self.it_num / self.params["links"]["ce"])
            * random.randn(*self.pts.shape)
        )

    def iter_(self):
        r = self.params["nodes"]["radius"] / 10.0
        self.get_distz()
        self.t += self.dt
        self.tv += [[self.t, self.lens()]]
        self.it_num += 1
    ...
    def make_links(net):
        ...
        for k in net.elist:
            ...
            lnks[str("%s;%s" % (nl[k0[0]], nl[k0[1]]))] = {  # str(k0)
                ...
                "radius": net.params["links"]["thickness"]
                * (1 if len(k) < 3 else k[2]),
                "weight": 1,
            }
        return lnks
    ...
    def net_json(net):
        net.JSON = {
            ...
            "nodes": {
                ...
                "labels": net.params["nodes"]["labels"]
                # range(len(net.pts))
            },
            "info": net.params,
        }
    ...
    def save(net, path, tv=True):
        fnam = path + "/" + net.gnam
        net.params["links"]["thickness"] = 0.1 * net.params["nodes"]["radius"]
        net.net_json()  # net.net_json()
        json.dump(net.JSON, open(fnam + ".json", "w"))
        if tv:
            savetxt(fnam + "-tv.txt", net.tv)
    ...
    **kw
    ...
    if simple:
        ...
        for i in nn.links.idx.values():
            plot(
                *l[i[0] : i[1] + 1, dims].T, c=link_col, marker="", ls="-"
            )  # , c = 'r')
...
def check_convergence(self, tol=0.01, win=500, **kw):
    ...
    return (ch * ch2 * ch3) or (abs(a1.mean()) < 0.5 * tol), a1.mean(), (ch, ch2, ch3)
...
def iter_converge(
    ...
    **kw
    ...
    r3 = mean((self.params["nodes"]["radius"] * degrees) ** 3) ** (
        1.0 / 3
    )  # / self.pts.shape[0]
    gnam0 = self.gnam + ""
    if its == "auto":
    ...
    while (self.it_num < max_its) and not ck:
        ...
        self.params["long_rep"] = 1e-3 + 100.0 / (self.it_num + 1)
        ct = (
            eps * max([r3, self.pts.std() / self.pts.shape[0] ** (1.0 / 3)])
            + 10.0 / (self.it_num + 1)
            if simple
            else "auto"
        )
...
class networkBase:
    def __init__(
        ...
        **kw
        ...
        self.params = {
            ...
            "nodes": {"amplitude": 5e2, "radius": 1.0, "weighted": 1},
        }
        ...
        if not JSON:
            ...
            self.elist = array(edg, dtype=int32)[
                :, :2
            ]  # tf.Variable(edg, dtype=tf.float32)
            self.link_weights = (
                array(edg)[:, 2]
                if self.params["links"]["weighted"]
                else array([1] * len(self.elist))
            )
    ...
    def get_JSON(self, JSON, kw):
        ...
        wg = (
            self.params["links"]["weighted"]
            if "weighted" in self.params["links"]
            else False
        )
        ...
        self.asg["dt"] = (
            self.dt.assign(
                ...
                * tf.minimum(
                    mx,
                    tf.minimum(
                        self.th_mean / self.f_link_max, self.nrad_mean / self.f_node_max
                    ),
                )
            ),
        )
    ...
    def get_ct(self, w=200, c0=0.5, eps_ct=1e-4, **kw):
        if len(self.tv) < w:
            self.ct = c0
            return 0
        i = array(self.tv[-w:])[:, 1]
        ...
        gbeg, gend = (
            i[w0 : 2 * w0].mean() - i[:w0].mean(),
            i[-w0:].mean() - i[-2 * w0 : -w0].mean(),
        )
        # If change in grad is small, increase ct factor
    ...
    def iter_all(
        self, it=1, ellipsoid=False, **kw
    ):  # rebin = 'auto' , bin_size = 10, ct='auto',  **kw):
        """Calls
        ...
        for i in arange(it):
            ...
            self.step_points.run(
                {self.ct_: self.ct, self.links.noise_: self.links.thermal_noise()}
            )
    ...
    def make_link_labels(self):
        ...
        self.params["links"]["labels"] = [
            "%s; %s" % (n[i], n[j]) for i, j in self.elist
        ]
    ...
    def make_JSON(self):
        ...
        self.netJSON["nodes"]["labels"] = (
            nd["labels"]
            if "labels" in nd
            else [i for i in range(len(self.netJSON["nodes"]["positions"]))]
        )
        ld = self.params["links"]
        self.netJSON["links"]["labels"] = (
            ld["labels"] if "labels" in ld else [i for i in range(len(self.elist))]
        )
        th = self.links.thickness[
            :: self.links.segs
        ].tolist()  # self.params['links']['thickness']
        links = {}
    ...
    def save(self, path="./", tv=True):
        ...
        name = self.netJSON[
            "name"
        ]  # + ('/' if self.netJSON['name'][-1] is not '/' else '')
        json.dump(self.netJSON, open(path + name + ".json", "w"))
        if tv:
            savetxt(path + name + "-tv.txt", self.tv)
...
class NetNodes:
    ...
    def __init__(nodes, net, **kw):
        ...
        if not nodes.net.fixed:
            nodes.get_r0()
            nodes.Force_repel = BinnedCompute(
                nodes, nodes.force_node_repel_idx, nodes.net.max_workers
            )

    def get_r0(nodes):
        nodes.r0 = float32(
            (nodes.degrees ** 1.0 if nodes.weighted else ones_like(nodes.degrees))
            * nodes.radius
        )
    ...
    def get_degreesRS(nodes):
        ...
        nodes.degrees = array(
            [
                (rs(nodes.deg_dict[i]) if i in nodes.deg_dict else 0.001)
                for i in nodes.labels
            ]
        )
    ...
    def dyn_setup(nodes):
        ...
        nodes.net.asg["N"] = (
            (nodes.points.assign_add(nodes.dp),) if not nodes.net.fixed else tuple()
        )
    ...
    def force_end(nodes):
        ...
        nodes.Force_End = tf.concat(
            [
                [
                    tf.add(
                        array([0, 0, 0], dtype=float32),
                        tf.reduce_sum(
                            [nodes.net.links.Force_Int[j] for j in nodes.link_ends[k]],
                            0,
                        ),
                    )
                ]
                for k in range(len(nodes.net.pts))
            ],
            0,
        )
...
class NetLinks:
    def __init__(links, net=None, **kw):
        ...
        links.Force_ext = BinnedCompute(
            links, links.forces_ext_brute_idx_multi, links.net.max_workers
        )
        links.setup_v_rot_()
        links.Force_ext_ell = BinnedCompute(
            links, links.force_ext_ellipsoid_idx_multi, links.net.max_workers
        )
        links.dyn_setup()
    ...
    def make_attr_pts(links, atr, dtype=float32):
        try:
            ...
            setattr(
                ...
                atr,
                array(
                    [getattr(links, atr)] * len(links.net.elist) * links.segs,
                    dtype=dtype,
                ),
            )
    ...
    def force_internal(links):
        ...
        Beware of that!!"""
        end_pts = array(
            [i for i in links.idx.values()], dtype=int32
        )  # to accelerate correcting endpoint forces
    ...
    def setup_v_rot_(links):
        ...
        ri = tf.concat(
            (dlh[:, :, newaxis], r1h[:, :, newaxis], r2h[:, :, newaxis]), axis=2
        )
    ...
    def get_dr2(links, r, gij):
        ...
        return tf.matmul(dr[:, :, newaxis], tf.matmul(gij, dr[:, :, :, newaxis]))[
            :, :, 0
        ]
    ...
    def dyn_setup(links):
        ...
        links.net.f_link_max = tf.reduce_max(
            tf.abs(links.net.f_link * links.end_mask_full)
        )
...
class bin_pts:
    ...
    def prep_bins(self):
        ...
        l = tf.minimum(
            (tf.reduce_max(self.obj.points) - tf.reduce_min(self.obj.points)) / 3, l0
        )
        self.cell_size = l
        self.cells_shift_ = tf.placeholder(
            tf.float32, shape=(self.obj.points.shape.as_list()[1],)
        )
        self.cells_raw = tf.to_int32(
            tf.floor(self.obj.points[:-1] / l + self.cells_shift_)
        )
        # cells_raw : array of int coords for points
        self.bins = {}

    def bin_rand(self, bin_size=10.0):  # split = links.segs
        d = self.obj.points.shape.as_list()[1]
        x = self.cells_raw.eval(
            {self.cells_shift_: random.randn(d), self.bin_size_: bin_size}
        )
        cells = self.hash_(x)
    ...
    def auto_rebin(
        self, min_cells=5, min_step=40, **kw  # self.links.max_workers
    ):  # ,c0 = 0.03
        """
...
class BinnedCompute:
    ...
    def iter_full(self):
        ...
        for i in range(0, len(self.obj.Bins.curr_keys), m):
            ...
            m0 = len(ix)
            self.obj.net.session.run(
                self.tensors[:m0],
                feed_dict={
                    self.idx_[i]: self.obj.Bins.bins[v] for i, v in enumerate(ix)
                },
            )
